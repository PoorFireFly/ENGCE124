...
void A1(int i, int x) { //Put data into Array 1 Dimension
 p = BA1+(i-l); //Calculate pointer BA1+1
 *p = x; //Put data 
} 
...
void Create2DArray() { 
 ...
 element = (u1-l1+1)*(u2-l2+1); // (3-1+1)*(4-1+1) = 12
 c = sizeof(*BA2); //4
 total_mem = element*c; //12 * 4 = 48byte
 BA2 = (int*)malloc(total_mem); 
} 
void A2(int i, int j, int x) { 
 p = BA2+((i-l1)*(u2-l2+1)+(j-l2)); //BA2+((2-1)*(4-1+1)+(3-1))
 *p = x; 
} 
...
void Create3DArray() { 
 int element, c, total_mem; 
 element = (u1-l1+1)*(u2-l2+1)*(u3-l3+1); //(3-1+1)*(4-1+1)*(5-1+1) = 60
 c = sizeof(*BA3); //4
 total_mem = element*c; //60*4 = 240byte 
